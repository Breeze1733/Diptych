...
class NotificationService {
  ...
  /// 读取本地保存的通知列表（按时间倒序）
  static Future<List<AppNotification>> loadLocalNotifications(
    String userId,
  ) async {
  ...
  static Future<List<AppNotification>> sync(
    ...
    // 1. 先从服务端拉取待读通知（服务端在读取时已自动清空该批队列）
    final pending = await api.getPendingNotifications(userId);

    // 2. 关键：在网络请求返回后，重新读取本地最新的通知列表！
    // 避免在异步请求耗时期间用户执行了「全部已读」或「单条已读」，导致修改被旧的 localList 覆盖
    final currentLocalList = await loadLocalNotifications(userId);
    if (pending.isEmpty) return currentLocalList;

    // 3. 合并入本地列表（若 ID 已存在则原地更新内容，保留本地最新的 isRead 状态；新 ID 则以未读状态追加到最前）
    final existingMap = {for (final n in currentLocalList) n.id: n};
